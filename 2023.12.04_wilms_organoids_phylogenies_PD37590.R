...
patient <- 'PD37590' # can code this so can come in from the command line
...
# I could make a pseudo normal by merging crypts from different sites. 

table(crypts[crypts$patient=='PD37590','site'])
head(crypts)
...
filtmat <- aa30[,c('mutkey', paste0(rep(allsamps, each=2), c('_MTR', '_DEP')))]
...
hist(filtmat$norm_dep, 100, xlim=c(0,2000))
abline(v=1000, col='red')
abline(v=2000, col='red')
norm_cutoff_min <- 1000
norm_cutoff_max <- 2000
...
filtmat$cov_in_tumour[(!grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>250 & filtmat$tumour_dep<600] <- 'good'
filtmat$cov_in_tumour[(grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>250 & filtmat$tumour_dep<600] <- 'good'

# I don't need to worry about CN for this patient
...
plot(atree, font=2, direction='downwards', tip.color=tipcols, label.offset=1, cex=1)
