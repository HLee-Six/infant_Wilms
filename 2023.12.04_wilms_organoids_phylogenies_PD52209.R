...
patient <- 'PD52209' # can code this so can come in from the command line
setwd(paste0(getwd(), '/', patient))

# read in a spreadsheet that has the patient details: if organoids or LCM, which samples are tumour samples vs normal, etc.
...
# get a list of the clonal samples - this is all of them except for a, b, c, and e
# a and c are tumour
# b is normal kidney
# e is normal blood
# I am calling them 'tum_orgs' even though they are microbiopsies for consistency with the other patients
samps <- gsub("_VAF","",intersect(grep("VAF", colnames(aa30), value=T), grep("PD52209", colnames(aa30), value=T)))
samps <- samps[!samps %in% c('PD52209b', 'PD52209e', "PD52209c")] # PD52209b is normal kidney
# there are no clonal normal samples. 
tum_orgs <- samps
normals <- c("PD52209b", "PD52209e") 
# there are no clonal normal samples. I.e. no 'norm_orgs' for this patient
...
hist(filtmat$tumour_dep, 100, xlim=c(0, 2500))

par(mfrow=c(2,1))
hist(filtmat$tumour_dep[grep('X|Y',filtmat$mutkey, invert = T)], 100, xlim=c(0,2500), main='autosomal mutations', xlab='depth')
hist(filtmat$tumour_dep[grep('X|Y',filtmat$mutkey)], 100, xlim=c(0,2500), main='sex chr mutations', xlab='depth')
# this is a male, so there is a difference between sex chroms and autosomes. 

# I do need to worry about CN for this patient
ascbins <- read.csv('ascat_files/PD52209_ascat_calls.csv', header=T, stringsAsFactors = F)
head(ascbins)
ascbins$X <- NULL
table(ascbins$all_calls)

# only use ones that have the same copy number state in all samples for phylogeny construction
filtmat$cn_state <- 'unknown'
dim(ascbins)
dim(filtmat)

for (bin  in 1:nrow(ascbins)) {
  if (bin %% 100 == 0) print(bin)
  inbin <- sapply(strsplit(filtmat$mutkey, '_'), '[[', 1) == ascbins[bin,'chrom'] & sapply(strsplit(filtmat$mutkey, '_'), '[[', 2) >= ascbins[bin,'start'] & sapply(strsplit(filtmat$mutkey, '_'), '[[', 2) < ascbins[bin,'end']
  filtmat[inbin,'cn_state'] <- ascbins[bin,'all_calls']
}
head(filtmat)
table(filtmat$cn_state)
write.table(filtmat, 'filters_outcome_with_cn_state.txt', sep='\t', col.names = T, row.names=F, quote=F)

filtmat$cov_in_tumour <- 'bad'
filtmat$cov_in_tumour[(!grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>700 & filtmat$tumour_dep<1500] <- 'good'
filtmat$cov_in_tumour[(grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>350 & filtmat$tumour_dep<750] <- 'good'
...
if (chars_to_add < 0) {
  rtree$tip.label[rtree$tip.label!='root000000'] <- paste0(substr(patient, 1, 5), rtree$tip.label[rtree$tip.label!='root000000'])
}
...
### don't need to worry about in vitro mutations as an lcm case
