...
# this one script is meant to work for all the phylogenies. 
...
patient <- 'PD52230' # can code this so can come in from the command line
...
# get a list of the clonal samples - this is all of them except for a, b, d, and f
# a, d, and e are tumour
# b is normal kidney
# f is normal blood
# I am calling them 'tum_orgs' even though they are microbiopsies for consistency with the other patients
samps <- gsub("_VAF","",intersect(grep("VAF", colnames(aa30), value=T), grep("PD52230", colnames(aa30), value=T)))
samps <- samps[!samps %in% c('PD52230b', 'PD52230f', "PD52230a", 'PD52230d',  'PD52230e')] # PD52230b is normal kidney
normals <- c("PD52230b", "PD52230f") 
tum_orgs <- samps
...
ascbins <- read.csv('ascat_files/PD52230_ascat_calls.csv', header=T, stringsAsFactors = F)
head(ascbins)
ascbins$X <- NULL
...
filtmat$cov_in_tumour[(!grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>1000 & filtmat$tumour_dep<1750] <- 'good'
filtmat$cov_in_tumour[(grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>1000 & filtmat$tumour_dep<1750] <- 'good'
