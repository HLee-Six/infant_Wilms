...
aa10 <- read.csv('PD54846_mq10bq10.tsv', sep='\t', header=T, stringsAsFactors = F)
aa30 <- read.csv('PD54846_mq30bq30.tsv', sep='\t', header=T, stringsAsFactors = F)
...
# get a list of the clonal samples - this is all of them except for the blood. 
# I am calling them 'tum_orgs' even though they are microbiopsies for consistency with the other patients
samps <- gsub("_VAF","",intersect(grep("VAF", colnames(aa30), value=T), grep("PD54846", colnames(aa30), value=T)))
samps <- samps[samps!='PD54846m']
# there are no clonal normal samples. 
...
write.table(gdsom, 'PD54846_mq30bq30_muts_pass_germline_and_betabinom_filters.txt',  ######################### change line with different patients
...
pdf("PD54846_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.pdf")
plot(tree_estimated)
axisPhylo(side=1, backward=F)
dev.off()
write.tree(tree_estimated, 'PD54846_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.tree')
...
# as these are not organoids, there are no in vitro mutations to filter out. 
mmres$invitro <- FALSE
invitro_pval <- 0.1
...
pdf('PD54846_phylogeny_in_vitro_mutations_removed_no_trunk.pdf')
...
filtmat$tumour_mtr <- rowSums(aa30[,paste0(samps, '_MTR')])
filtmat$tumour_dep <- rowSums(aa30[,paste0(samps, '_DEP')])
...
filtmat$cov_in_tumour[(!grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>500 & filtmat$tumour_dep<1000] <- 'good'
filtmat$cov_in_tumour[(grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>150 & filtmat$tumour_dep<1000] <- 'good'
...
# in this case the truncal mutas are being filtered out by the betabinomial filter - I have no normal clonal samples for them to be negative in
# I could also be filtering out mutations that are shared between all the a samples or all the d samples - these could have low vaf in the combined tumour samples.
dsamps <- grep("d", samps, value=T)
asamps <- grep("a", samps, value=T)

filtmat$d_mtr <- rowSums(aa30[,paste0(dsamps, '_MTR')])
filtmat$d_dep <- rowSums(aa30[,paste0(dsamps, '_DEP')])
filtmat$d_vaf <- filtmat$d_mtr/filtmat$d_dep

filtmat$a_mtr <- rowSums(aa30[,paste0(asamps, '_MTR')])
filtmat$a_dep <- rowSums(aa30[,paste0(asamps, '_DEP')])
filtmat$a_vaf <- filtmat$a_mtr/filtmat$a_dep

pdf("PD54856_a_and_b_separately.pdf")
plot(d_vaf ~ a_vaf, data=filtmat[(!grepl('X|Y',filtmat$mutkey) & filtmat$norm_vaf<0.3),])
plot(d_vaf ~ a_vaf, data=filtmat[(grepl('X|Y',filtmat$mutkey)),])
plot(d_vaf ~ norm_vaf, data=filtmat[(!grepl('X|Y',filtmat$mutkey)),], pch=16, cex=0.3)
points(tumour_vaf~norm_vaf, data=filtmat[filtmat$mutkey %in% tum_subclonal_muts,], 
       pch=1, cex=1.2, col='brown', main='all muts good coverage')
plot(a_vaf ~ norm_vaf, data=filtmat[(!grepl('X|Y',filtmat$mutkey)),], pch=16, cex=0.3)
points(tumour_vaf~norm_vaf, data=filtmat[filtmat$mutkey %in% tum_subclonal_muts,], 
       pch=1, cex=1.2, col='brown', main='all muts good coverage')
dev.off()
...
in_tum_failed_betabinom <- filtmat[filtmat$pass_betabinom==FALSE & filtmat$tumour_vaf>0.25 & filtmat$clonal_in_normals=='no',]
...
# of these 12, after manual review on jbrowse, 9 are good truncal mutations. Read back in. 


##############
# finalise set of mutations
# read them back in and add them on to the trunk of the tumour.
in_tum_failed_betabinom_qced <- read.csv("subclonal_in_norm_clonal_in_tumour_failed_betabinom_to_check.csv", header=T, stringsAsFactors = F)
head(in_tum_failed_betabinom_qced)
trunk_to_add <- in_tum_failed_betabinom_qced[in_tum_failed_betabinom_qced$jbrowse=='good','mutkey']

aa30[aa30$mutkey %in%  trunk_to_add, paste0(samps, '_VAF')] # these are all truncal - in every tumour sample, and in both a and d sides of the tree.

# add these extra mutations to the tree trunk.
plot(atree, use.edge.length=F)
nodelabels()
tiplabels()
# I want to add to the one that leads to the MRCA of the tumour. So find the MRCA of the tumour. 
node_descendants <- sapply(atree$edge[,2], function(node) getDescendants.fn(atree, node))
tumour_mrca <- atree$edge[,2][which(sapply(node_descendants, function(descs) all(tum_orgs %in% descs)))]
original_tum_trunk_length <- atree$edge.length[which(atree$edge[,2]==tumour_mrca)]
new_tum_trunk_length <- original_tum_trunk_length + length(trunk_to_add)

btree <- atree
btree$edge.length[which(atree$edge[,2]==tumour_mrca)] <- new_tum_trunk_length
plot(btree)

write.tree(btree, 'PD54846_tree_failed_betabinom_salvaged.tree')

pdf('PD54846_phylogeny_failed_betabinom_salvaged.pdf')
...
aa30$pass_betabinomial_filter[aa30$mutkey %in% trunk_to_add] <- 'salvaged_by_jbrowse'
...
table(mmres$edge_ml)
muts_with_status[muts_with_status$pass_betabinomial_filter=='salvaged_by_jbrowse','edge_ml'] <- 1
muts_with_status[muts_with_status$pass_betabinomial_filter=='salvaged_by_jbrowse','descendant_tips'] <- paste0(sort(samps), collapse=';')
...
write.table(muts, 'PD54846_pileup_with_filters_assignments_trinucs.txt', sep='\t', col.names=T, row.names=F, quote=F)
...
pdf('PD54846 trinucleotide plots by filter and by branch.pdf')
par(mfrow=c(2,1))
...
# muts <- read.csv('PD54846_pileup_with_filters_assignments_trinucs.txt', sep='\t', header=T, stringsAsFactors = F)

# for every branch plot clonal and subclonal mutations
samp <- samps[1]

pdf('PD54846_vafs_shared_vs_private.pdf')
par(mfrow=c(2,1))
for (samp in samps) {
