...
aa10 <- read.csv('PD37585_mq10bq10.tsv', sep='\t', header=T, stringsAsFactors = F)
aa30 <- read.csv('PD37585_mq30bq30.tsv', sep='\t', header=T, stringsAsFactors = F)
...
tum_orgs <- c('PD37585a', 'PD37585c', 'PD37585d', 'PD37585e', 'PD37585f', 'PD37585g', 'PD37585h', 'PD37585i', 'PD37585j')
norm_orgs <- c('PD37585b')
...
# In PD37585, I have both blood and perinephric fat as bulk normal samples. Combine them for higher sequencing depth. 
normals <- c("PD37585s", "PD37585t")
...
hist(filtmat$norm_dep, 100, xlim=c(0,250))
abline(v=50, col='red')
...
write.table(gdsom, 'PD37585_mq30bq30_muts_pass_germline_and_betabinom_filters.txt',  ######################### change line with different patients
...
pdf("PD37585_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.pdf")
plot(tree_estimated)
axisPhylo(side=1, backward=F)
dev.off()
write.tree(tree_estimated, 'PD37585_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.tree')
...
tipcols[atree$tip.label %in% c('PD37585b')] <- 'darkgrey'
tipcols[atree$tip.label %in% c('fertilised_egg')] <- 'black'

pdf('PD37585_phylogeny_in_vitro_mutations_removed_no_trunk.pdf')
...
# as in this sample where there is a clonal normal sample (PD37585b), there are two ways I can do this.
...
# bind on the b sample.
filtmat <- as.data.frame(cbind(filtmat, aa30[,c('PD37585b_MTR', 'PD37585b_DEP', 'PD37585b_VAF')]))
...
tum_trunk_muts <- mmres[mmres$descendant_tips=='PD37585a;PD37585c;PD37585d;PD37585e;PD37585f;PD37585g;PD37585h;PD37585i;PD37585j','mutkey']
tum_subclonal_muts <- mmres[!mmres$descendant_tips %in% c('PD37585a;PD37585c;PD37585d;PD37585e;PD37585f;PD37585g;PD37585h;PD37585i;PD37585j', 'PD37585b'),'mutkey']
...
# After inspecting them on jbrowse I think the high vaf ones filtered out by the betabinomial are actually real. 
...
trunk_to_add <- in_tum_failed_betabinom_qced[in_tum_failed_betabinom_qced$jbrowse.check=='good','mutkey']

aa30[aa30$mutkey %in%  trunk_to_add, paste0(samps, '_VAF')] # these are all truncal - in every tumour sample.
...
tumour_mrca <- atree$edge[,2][which(sapply(node_descendants, function(descs) all(tum_orgs %in% descs) & !any(descs %in% norm_orgs)))]
...
write.tree(btree, 'PD37585_tree_in_vitro_removed_failed_betabinom_salvaged.tree')

pdf('PD37585_phylogeny_in_vitro_mutations_removed_failed_betabinom_salvaged.pdf')
...
muts_with_status[muts_with_status$pass_betabinomial_filter=='salvaged_by_jbrowse','edge_ml'] <- 2
muts_with_status[muts_with_status$pass_betabinomial_filter=='salvaged_by_jbrowse','descendant_tips'] <- 'PD37585a;PD37585c;PD37585d;PD37585e;PD37585f;PD37585g;PD37585h;PD37585i;PD37585j'
...
write.table(muts, 'PD37585_pileup_with_filters_assignments_trinucs.txt', sep='\t', col.names=T, row.names=F, quote=F)
...
pdf('PD37585 trinucleotide plots by filter and by branch.pdf')
par(mfrow=c(2,1))
