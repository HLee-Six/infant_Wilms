...
setwd('/Users/hl11/Documents/Research/Projects/wilms_organoids/phylogenies_same_method_sept22/PD26636_crc/')

# read in the two pileups.
aa10 <- read.csv('PD26636_mq10bq10.tsv', sep='\t', header=T, stringsAsFactors = F)
aa30 <- read.csv('PD26636_mq30bq30.tsv', sep='\t', header=T, stringsAsFactors = F)
...
allsamps <- gsub('_VAF', '', grep('VAF', colnames(aa10), value=T))
samps <- allsamps[grepl('3$|4$|5$|6$', allsamps)]
tum_orgs <- samps[!grepl('b', samps)]
norm_orgs <- samps[grepl('b', samps)]
...
hist(filtmat$norm_dep, 100, xlim=c(0,150))
abline(v=30, col='red')
...
# deal with regions of copy number change. 
# a samples have undergone a wgd
# the c samples have not undergone a wgd but have lots of regions of change. 
# the e samples are relatively bland
# if I just keep regions that are heterozygous diploid, I will be left with no genome. 
# perhaps the best thing is to remove regions with a deletion. 
# Remove positions with min cn == 0. 

asc <- read.csv('PD26636_ascat_concat.csv', header=F, stringsAsFactors = F)
asc$sample <- sapply(strsplit(asc$V1, '.asc'), '[[', 1)
asc$V1 <- NULL
scasc <- asc[asc$sample %in% samps,]
toremove <- scasc[scasc$V8==0,]

gr <- GRanges(seqnames = toremove$V2, IRanges(start=toremove$V3, end=toremove$V4))
gr1 <- reduce(gr)
regions_to_cull <- as.data.frame(gr1)
propculled <- sum(regions_to_cull$width)/(3.2e+09) # 40% of the genome is removed. 

# now remove mutations that fall in this - annotate filtmat
filtmat$chrom <- sapply(strsplit(filtmat$mutkey, "_"), '[[', 1)
filtmat$pos <- as.numeric(sapply(strsplit(filtmat$mutkey, "_"), '[[', 2))
table(regions_to_cull$seqnames)
filtmat$deleted_region <- FALSE

for (i in 1:nrow(regions_to_cull)) {
  if (i%%10==0) print(i)
  
  tchrom <- regions_to_cull[i,'seqnames']
  tstart <- regions_to_cull[i,'start']
  tend <- regions_to_cull[i,'end']
  
  if (nrow(filtmat[filtmat$chrom==tchrom & filtmat$pos>=tstart & filtmat$pos<=tend,])>0) {
    filtmat[filtmat$chrom==tchrom & filtmat$pos>=tstart & filtmat$pos<=tend,'deleted_region'] <- TRUE
  }
}

# call mutations for the purpose of building the tree.
gdsom <- aa30[aa30$mutkey %in% filtmat$mutkey[filtmat$assign_to_tumour_tree==TRUE & filtmat$deleted_region==FALSE],]
...
rtree <- tree
rtree$tip.label[rtree$tip.label!='root000000'] <- gsub('0','', rtree$tip.label[rtree$tip.label!='root000000'])
...
sort(colSums(toassign)) # ranges from 36 to 167 mutations 
head(toassign)
...
pdf("PD26636_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.pdf")
plot(tree_estimated)
axisPhylo(side=1, backward=F)
dev.off()
write.tree(tree_estimated, 'PD26636_tree_built_with_muts_not_clonal_in_normal_in_vitro_not_filtered.tree')
...
### in vitro mutations
# for clonal samples, I can just use the vaf to remove these. 
# but I have kept in lots of copy number changes. 
# for each mutations, I should find the the total copy number that ascat has called at that locus.
# I should use 1/(total CN) as the expected centre of the binomial distribution.
...
# here i adjust for CN state.
  # I think I do the same as for all the other patients, but I break each sample down by copy number.
  tcn_states <- sort(unique(asc[asc$sample==samp & asc$V8!=0,'V7']))
  
  for (cn in tcn_states) {
    print(cn)
    
    # set the target VAF
    tvaf <- 1/cn
    
    # get the mutations in regions of this total cn state
    tregions <- asc[asc$sample==samp & asc$V8!=0 & asc$V7==cn,]
    
    tcnaa30 <- data.frame()
    for (i in 1:nrow(tregions)) {
      tchrom <- tregions[i,'V2']
      tstart <- tregions[i,'V3']
      tend <- tregions[i,'V4']
      
      taa30 <- aa30[aa30$Chrom==tchrom & aa30$Pos >= tstart & aa30$Pos <= tend,]
      tcnaa30 <- as.data.frame(rbind(tcnaa30, taa30))
    }

    # work out the probability of a private mutation being clonal.
    tmtrs <- tcnaa30[tcnaa30$mutkey %in% tprivate$mutkey, paste0(samp, '_MTR')]
    tdeps <- tcnaa30[tcnaa30$mutkey %in% tprivate$mutkey, paste0(samp, '_DEP')]
    tmuts <- as.data.frame(cbind(tmtrs, tdeps))
    tmuts$tvafs <- tmtrs/tdeps
    tmuts$mutkey <- tcnaa30[tcnaa30$mutkey %in% tprivate$mutkey, 'mutkey']
    ...
    if (length(tcnaa30[tcnaa30$mutkey %in% tshared$mutkey, paste0(samp, '_VAF')]) >0) {
      hist(tcnaa30[tcnaa30$mutkey %in% tshared$mutkey, paste0(samp, '_VAF')], xlim=c(0,1), 50, 
           main=paste0(samp, ' shared mutations \n CN state ', cn), xlab='VAF')
      abline(v=tvaf, col='red', lwd=2)
    } else {
      plot(1~1, type='n', axes=F, xlab='', ylab='')
      text(1,1, 'No shared mutations')
    }
    hist(tcnaa30[tcnaa30$mutkey %in% tprivate$mutkey, paste0(samp, '_VAF')], xlim=c(0,1), 50, 
         main=paste0(samp, ' private mutations \n ', length(tprivate$mutkey), ' subs unfiltered \n CN state ', cn), xlab='VAF')
    abline(v=tvaf, col='red', lwd=2)
    hist(tcnaa30[tcnaa30$mutkey %in% tmuts$mutkey[tmuts$prob_vaf_this_low>invitro_pval], paste0(samp, '_VAF')], xlim=c(0,1), 50, 
         main=paste0(samp, ' private mutations \n ', length(tmuts$mutkey[tmuts$prob_vaf_this_low>invitro_pval]), ' subs after filtering \n CN state ', cn), xlab='VAF')
    abline(v=tvaf, col='red', lwd=2)
...
### and, specifically for this patient, need to adjust branch lengths for mutations lost due to CN changes
### as had to exclude 40% of the genome, divide all branches by (1-propculled)
...
for (edgeml in unique(mmres$edge_ml)) {
  atree$edge.length[as.numeric(edgeml)] <- round((nrow(mmres[mmres$edge_ml==edgeml & mmres$invitro==FALSE,])/(1-propculled)))
}
...
pdf('PD26636_phylogeny_in_vitro_mutations_removed_no_trunk_adjusted_for_removal_of_deleted_regions.pdf')
plot(atree, font=2, direction='downwards', tip.color=tipcols, label.offset=10, cex=1)
axisPhylo(side=2, backward = F)
dev.off()

write.tree(atree, "PD26636_phylogeny_in_vitro_mutations_removed_adj_for_deleted_regions_betabinom_salvaged.tree")
...
hist(filtmat$tumour_dep, 100, xlim=c(0,600))
abline(v=100, col='blue')
abline(v=600, col='blue')
filtmat$cov_in_tumour <- 'bad'
filtmat$cov_in_tumour[filtmat$tumour_dep>100 & filtmat$tumour_dep<600] <- 'good'

# bind on the normal organoids.
head(filtmat)
filtmat <- as.data.frame(cbind(filtmat, aa30[,paste0(rep(norm_orgs, each=3), c('_MTR', '_DEP', '_VAF'))]))
...
tum_trunk_muts <- mmres[mmres$descendant_tips=='PD26636a3;PD26636a6;PD26636c3;PD26636c4;PD26636d4;PD26636d6;PD26636e3;PD26636e4','mutkey']
tum_subclonal_muts <- mmres[!mmres$descendant_tips %in% c('PD26636a3;PD26636a6;PD26636c3;PD26636c4;PD26636d4;PD26636d6;PD26636e3;PD26636e4', 'PD37585b'),'mutkey']
...
# it's going to be harder to pick out embryonic mutations using tumour vaf due to the cn changes in the tumour
...
# looking at this plot, I think it's going to be really hard to separate early embryonic mutations from germline.
# and it does not look like there will be many extra embryonic mutations - perhaps a handful.

# finalise set of mutations
# there were none to salvage from the betabinoma
...
head(muts_with_status)


# also need to annotate if I removed mutations because they were in a region that had undergone a copy number change
muts_with_status$deleted_region <- FALSE
muts_with_status[muts_with_status$mutkey %in% filtmat$mutkey[filtmat$deleted_region==TRUE],'deleted_region'] <- TRUE
head(muts_with_status)
...
write.table(muts, 'PD26636_pileup_with_filters_assignments_trinucs.txt', sep='\t', col.names=T, row.names=F, quote=F)
...
pdf('PD26636 trinucleotide plots by filter and by branch.pdf')
par(mfrow=c(2,1))
