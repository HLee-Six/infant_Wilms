...
patient <- 'PD26636' # can code this so can come in from the command line
...
unique(sapply(strsplit(colnames(aa30), '_'), '[[', 1))
tum_orgs <- c('PD26636a3', 'PD26636a6', 'PD26636c3', 'PD26636c4', 'PD26636d4', 'PD26636d6', 'PD26636e3', 'PD26636e4')
norm_orgs <- c('PD26636b3', 'PD26636b4', 'PD26636b5', 'PD26636b6')
...
hist(filtmat$norm_dep, 100, xlim=c(0,200))
abline(v=30, col='red')
...
hist(filtmat$tumour_dep, 100, xlim=c(0,800))

par(mfrow=c(2,1))
hist(filtmat$tumour_dep[grep('X|Y',filtmat$mutkey, invert = T)], 100, xlim=c(0,600), main='autosomal mutations', xlab='depth')
hist(filtmat$tumour_dep[grep('X|Y',filtmat$mutkey)], 100, xlim=c(0,600), main='sex chr mutations', xlab='depth')
# this is a male, so there is a difference between sex chroms and autosomes. 

filtmat$cov_in_tumour <- 'bad'
filtmat$cov_in_tumour[(!grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>100 & filtmat$tumour_dep<600] <- 'good'
filtmat$cov_in_tumour[(grepl('X|Y',filtmat$mutkey)) & filtmat$tumour_dep>50 & filtmat$tumour_dep<300] <- 'good'

#### do need to worry about cn states.
...
asc <- read.csv('ascat_files/PD26636_ascat_concat.csv', header=F, stringsAsFactors = F)
head(asc)
...
head(filtmat)
table(filtmat$deleted_region)

# for compatibility with the other cases, call non-deleted ones '2_1', even though they're not, of course
filtmat$cn_state <- '2_1'
filtmat$cn_state[filtmat$deleted_region==TRUE] <- ';'
...
rtree$tip.label[grep('PD26636', rtree$tip.label)] <- substr(rtree$tip.label[grep('PD26636', rtree$tip.label)], 1, 9)
